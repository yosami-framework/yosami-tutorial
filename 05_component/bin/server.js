...
const asset      = new Asset(path.resolve(__dirname, '..', 'public'), Manifest['app.js'], Manifest['assets/app.scss']);

const localeDir = path.resolve(__dirname, '..', 'config', 'locales');
const assetsDir = path.resolve(__dirname, '..', 'public', 'assets/');

fs.readdirSync(localeDir).filter((f) => /\.yml$/.test(f)).forEach((file) => {
  TrackI18n.load(require(Manifest[`assets/${file}`].replace(/^(.*?)\/assets/, assetsDir)));
});
